...
def main():
    parser = argparse.ArgumentParser(
        description="Collect Y-DNA haplogroup paths from FTDNA"
    )
    parser.add_argument(
        "--mode",
        choices=["update", "full"],
    ...
    input_json = "data/slo-ydna.json"
    output_json = "data/slo-ydna-paths.json"
    ...
    for idx, hg in enumerate(missing_hgs, 1):
        ...
        url = f"https://discover.familytreedna.com/resources/y-dna/{hg}.json"
        headers = {
                            ...
                            def find_path(obj):
                                if isinstance(obj, list):
                                    if (
                                        ...
                                        if hg in names or "A0000" in names:
                                            return obj
                                    for item in obj:
                                        res = find_path(item)
                                        if res is not None:
                        ...
                        if not path_data:
                            ...
                            for item in path_data:
                                ...
                                if node_name not in nodes_db:
                                    ...
                                    note = item.get("historicalEvent", "") or item.get(
                                        "note", ""
                                    )
                                    if not note:
                                        note = ""
    ...
    # Sort them by age (oldest/most negative first)
    def sort_key(n):
        age = n.get("age")
        if age is None:
            return (999999, n.get("haplogroup", ""))
        return (age, n.get("haplogroup", ""))
